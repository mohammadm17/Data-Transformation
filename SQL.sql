...
select driver_name,round(avg(score),2) as Average_Score from responses group by driver_name

Find all scores by department and driver combination
select b.department,a.driver_name,a.score from responses a join employees b on a.employee_id =  b.id 
 order by b.department asc , a.driver_name
 
 Find all scores by location and driver combination
 select b.location,a.driver_name,a.score from responses a join employees b on a.employee_id =  b.id 
 order by b.location asc , a.driver_name
 
 Find all scores by gender and drive
 select b.gender,a.driver_name,a.score from responses a join employees b on a.employee_id =  b.id 
 order by b.gender asc , a.driver_name
 
 Find all scores by age group 20-30, 30-40,40-50
 SELECT 
    a.score,
    CASE 
        WHEN b.age BETWEEN 21 AND 30 THEN '20-30' 
        WHEN b.age BETWEEN 31 AND 40 THEN '30-40' 
        WHEN b.age BETWEEN 41 AND 50 THEN '40-50' 
        -- Add more ranges if needed
    END AS age_group
FROM 
     responses a join employees b on a.employee_id =  b.id
order by age_group;

Find all scores by managers (ensure manager name is there)
select a.name, b.score from employees a join responses b on  b.employee_id =  a.id  where a.manager_id is null
...
FROM 
    responses a
JOIN 
    employees b ON a.employee_id = b.id
GROUP BY
    b.department,
    a.driver_name
HAVING 
